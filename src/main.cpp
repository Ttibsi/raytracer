...
color ray_color(const ray &r, const hittable &world, int depth) {
    // Set the colour of the pixel the ray is pointing at
    hit_record rec;
    ...
    return (1.0 - t) * color(1.0, 1.0, 1.0) + t * color(0.9, 0.7, 0.3);
}
...
hittable_list scene() {
    ...
    for (int a = -12; a < 12; ++a) {
        for (int b = -12; b < 12; ++b) {
            ...
            point3 center(a + 0.5 * random_double(), 0.1,
                          b + 0.5 * random_double());

            if ((center - point3(1, 0.2, 1)).length() > 0.9) {
                ...
                if (choose_mat < 0.8) {
                    ...
                    world.add(std::make_shared<Sphere>(
                        center, random_double(0.1, 0.4), sphere_material));
